...
Many constructive heuristics in this study first build an acyclic \emph{active} subgraph \(H=(V,A\setminus F)\) by deleting a set \(F\subseteq A\), and then extract a linear extension \(\pi\) of \(H\). For such a pair \((F,\pi)\), define
\begin{equation}
    B_{\pi}
    =
    \{(u,v)\in A:\ \pi(v)<\pi(u)\}.
...
\end{equation}
Equality need not hold: topological extraction is generally \emph{not} objective-neutral, because different linear extensions of the same active DAG can yield different values of \(\operatorname{bw}(\pi)\). The implementation therefore reports the ordering objective recomputed from the returned ranking, not the removed-set weight \(w(F)\) alone. Online Resource~1 records the detailed extraction discussion and calibration checks.
