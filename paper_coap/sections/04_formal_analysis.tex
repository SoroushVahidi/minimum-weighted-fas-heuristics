...
Let \(G'=(V,A')\) be the current active acyclic graph with topological order \(\pi\). After every candidate removed arc has been processed once and reinserted whenever insertion preserves acyclicity, the remaining removed set \(F\) is inclusion-minimal in the feedback-arc-set sense:
\begin{equation}
    \forall e\in F,\quad G'+e \text{ is cyclic}.
    \label{eq:addback-minimality}
\end{equation}
Equivalently, for every \(e\in F\), restoring \(e\) to \(A'\) creates a directed cycle, or \(A'\cup\{e\}\) is cyclic.
...
\begin{proposition}[IPSNS incumbent non-worsening]
...
\end{equation}
where \(s\) is the number of eligible SCCs and \(c_S\) is the number of restricted cycle-reduction rounds. These are conservative implementation-level bounds; practical runtime depends strongly on SCC structure, density, and the selected iteration budget.
