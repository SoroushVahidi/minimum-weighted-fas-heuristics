...
## Pass 1 correction 2 (same day, 2026-06-17): author confirmation lifts the Supercomputing blocker

The author has explicitly confirmed that the Journal of Supercomputing manuscript ("Learning-Free Ranking from Pairwise Comparisons via Feedback-Arc-Set Pruning and Add-Back") is related but distinct and is **not a substantial overlap concern** for the present SN Computer Science manuscript. The manuscript file itself remains unavailable in this repository/workspace, so no independent text-level comparison was performed — this is recorded as an author-supplied fact, not an audit finding — but it is sufficient to lift this item as a submission blocker.

Files corrected: `docs/sncs_preparation_202606/OVERLAP_AND_DISCLOSURE_AUDIT.md` (verdict changed from "NOT READY: Supercomputing overlap audit blocked" to "READY FOR HUMAN REVIEW BEFORE SNCS SUBMISSION"), `docs/sncs_preparation_202606/SUBMISSION_PACKAGE_STATUS.md`, `docs/MANUSCRIPT_AND_ARTIFACT_STATUS.md`, `docs/sncs_preparation_202606/README.md`, `README.md`, `paper_sncs/submission/sncs_initial/README_SNCS_UPLOAD.md`, and `paper_sncs/declarations/statements_and_declarations.tex` ("Related manuscripts and prior author work": removed the "pending unresolved" framing, replaced with concise wording disclosing the COAP decline as a separate factual statement and the Supercomputing manuscript as related-but-distinct).

This is **not** a certification that the manuscript is ready for final journal upload. Pass 2 remains outstanding: add modern FAS/MWFAS references and related-work/baseline-selection paragraphs before actual SNCS submission.
