...
`Vahidi_SNCS_Source.zip` is a backup LaTeX source package. It contains exactly one `.tex` file, `Vahidi_SNCS_Main.tex`, plus required bibliography/style/class files and any figure files actually used by the manuscript. All files inside the ZIP are at the same folder level; there are no subfolders. Upload it only if Editorial Manager requests LaTeX source files.
