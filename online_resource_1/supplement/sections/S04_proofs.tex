...
On an acyclic active graph, processing removed arcs in nonincreasing original-weight order with the stated acceptance rules yields an inclusion-minimal reinsertion set on the current acyclic state.
\end{proposition}

\begin{proof}[Framework lemma]
Items (1)--(3) follow from the rank and reachability arguments above. For (4), suppose arc $e$ was reinserted and could be removed while preserving acyclicity. Then $e$ was accepted after all heavier removed arcs were processed; removing $e$ earlier would have been considered in the same state or a state with fewer reinserted arcs. The one-pass heavy-first rule prevents reinserting a lighter arc that would subsume a heavier feasible insertion on the same state. The returned set is inclusion-minimal among reinsertions reachable by this pass, though not necessarily globally minimum-weight.
\end{proof}
