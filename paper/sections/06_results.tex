...
The exact small-instance study supports a quality-validation narrative rather than a theoretical one. On the 57 standard nonnegative instances with \(n \le 20\), IPSNS is exactly optimal on 56 and has a mean gap of 0.0006\% from the exact optimum. LR-TA is optimal on 55 instances with mean gap 0.0590\%, and WMSF is optimal on 51 with mean gap 0.0961\%. These numbers show that the framework is not merely competitive in relative benchmark terms; on small instances where exact optimization is feasible, it is essentially exact in practice.
